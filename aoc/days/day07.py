...
def find_beam_splitter(reached_splitter_positions: set, grid: utils.Grid, start: utils.Point, graph: utils.Graph, running_beams_set: set | None = None) -> None:
    ...
    graph.add_connection(start, next_p, 0)
    while True:
        if grid[next_p] == '^':
        ...
        if grid.in_bounds(next_p_plus):
            if grid[next_p] != '^':
                graph.add_connection(next_p, next_p_plus, 0)
            next_p = next_p_plus
        else:
            break
...
def solve_part_b(input_data: str) -> Any:
    ...
    for i in range(grid.cols):
        ...
        timelines += graph.count_shortest_paths(start, end)
